import React from 'react'
import Header from '@/components/Header'
import Footer from '@/components/Footer'
import Logo from '@/components/Logo'

export const metadata = {
  title: 'हमारे बारे में | Akashvani Speaking',
  description: 'Akashvani Speaking - हरियाणा की सबसे तेज़ और विश्वसनीय हिंदी न्यूज़ पोर्टल।',
}

export default function AboutPage() {
  return (
    <>
      <Header />
      <main className="max-w-4xl mx-auto px-4 py-10">
        <div className="bg-white rounded-2xl shadow-sm p-8 md:p-12 animate-fade-in">
          <div className="text-center mb-10">
            <div className="flex justify-center mb-4">
              <div className="w-20 h-20 rounded-full border-3 border-brand-navy flex items-center justify-center bg-brand-navy">
                <div className="w-[85%] h-[85%] rounded-full border-2 border-white flex items-center justify-center">
                  <span className="font-poppins font-bold text-white text-lg tracking-wider">
                    A&S
                  </span>
                </div>
              </div>
            </div>
            <h1 className="text-3xl md:text-4xl font-bold font-yantramanav text-brand-navy mb-2">
              Akashvani Speaking
            </h1>
            <p className="text-gray-500 font-yantramanav text-lg">
              ईमानदार सोच - सच्ची खबरें
            </p>
          </div>

          <div className="space-y-6 text-gray-700 font-yantramanav text-base leading-relaxed">
            <p>
              <strong>Akashvani Speaking</strong> हरियाणा की एक प्रमुख डिजिटल न्यूज़ पोर्टल है जो 
              ताज़ा, सटीक और विश्वसनीय खबरें &quot;पॉइंट टू पॉइंट&quot; फॉर्मेट में पाठकों तक पहुंचाती है।
            </p>
            <p>
              हमारा मिशन है कि हर नागरिक को सच्ची और पारदर्शी खबरें मिलें। हम हरियाणा के हर जिले 
              से ताज़ा खबरें, लाइव अपडेट, और ब्रेकिंग न्यूज़ कवर करते हैं।
            </p>
            <p>
              हमारी टीम अनुभवी पत्रकारों से बनी है जो निष्पक्ष और ईमानदार पत्रकारिता में विश्वास 
              रखते हैं।
            </p>
          </div>

          <div className="mt-10 p-6 bg-brand-lightGray rounded-xl text-center">
            <p className="text-sm text-gray-500 font-poppins">Website Developed by</p>
            <p className="text-lg font-bold text-brand-navy font-poppins mt-1">
              Revonic Private Limited
            </p>
            <p className="text-sm text-gray-500 font-poppins mt-1">📞 8053936120</p>
          </div>
        </div>
      </main>
      <Footer />
    </>
  )
}