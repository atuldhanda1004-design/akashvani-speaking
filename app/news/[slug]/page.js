...
// 📸 THIS CODE SENDS THE PHOTO TO WHATSAPP & FACEBOOK
export async function generateMetadata({ params }) {
  ...
  // This calls our api/og/route.jsx to generate the final image with logo and headline
...
export default async function NewsDetailPage({ params }) {
  ...
  const showLiveUpdates = (news.is_trending || news.is_breaking) && Array.isArray(news.live_updates) && news.live_updates.length > 0
  ...
  return (
    <>
      ...
      <main className="max-w-3xl mx-auto bg-white min-h-screen shadow-sm pb-10">
        ...
        {/* 🖼️ Passing Role to Slider -> ImageWithWatermark */}
        <ImageSlider
          ...
          role={news.users?.role} // <-- Role Passed Here
        />
