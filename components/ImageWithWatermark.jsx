...
export default function ImageWithWatermark({
  ...
  className = '',
  fill = false,
  // 'cover' = fill box (crops) | 'contain' = full photo no crop
  fit = 'contain',
}) {
  const fallback =
  ...
  return (
    <div
      className={`relative overflow-hidden bg-gray-900 w-full h-full min-h-[120px] ${className}`}
    >
      {fill ? (
        <Image
          src={imgSrc}
          ...
          className={`${objectClass} bg-gray-900`}
          priority
        />
      ) : (
        <Image
          ...
          width={1200}
          height={675}
          className={`${objectClass} w-full h-full bg-gray-900`}
          priority
        />
      )}
