...
export default function AdminSidebar({ isAdmin = false, userName = 'Admin' }) {
  ...
  const adminOnlyItems = [
    ...
    { icon: Users, label: 'Reporters / Payout', href: '/admin/dashboard?tab=reporters' },
  ]
  ...
  return (
    <>
      {/* Mobile Menu Toggle */}
      <button onClick={() => setIsMobileOpen(true)} className="lg:hidden fixed top-3 left-3 z-50 bg-brand-primary text-white p-2 rounded-lg shadow-md">
      ...
      {/* Sidebar */}
      <aside className={`fixed lg:sticky top-0 left-0 h-screen bg-brand-secondary text-white w-72 flex-shrink-0 z-50 transition-transform duration-300 ${isMobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
          ...
          {menuItems.map((item) => (
            <Link key={item.href} href={item.href} onClick={() => setIsMobileOpen(false)}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-poppins transition-all ${
                isActive(item.href) ? 'bg-white text-brand-primary font-bold shadow' : item.highlight ? 'bg-brand-primary/50 text-white font-semibold hover:bg-brand-primary' : 'text-white/70 hover:bg-white/10'
              }`}>
          ...
          {isAdmin && (
            <>
              <div className="pt-4 pb-2">
                <p className="text-[10px] text-white/40 uppercase tracking-wider px-3">Admin Only</p>
              </div>
              {adminOnlyItems.map((item) => (
                <Link key={item.href} href={item.href} onClick={() => setIsMobileOpen(false)}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-poppins transition-all ${
                    isActive(item.href) ? 'bg-white text-brand-primary font-bold shadow' : 'text-white/70 hover:bg-white/10'
                  }`}>
                  <item.icon className="w-4 h-4" /> {item.label}
                </Link>
              ))}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3 border-t border-white/10 bg-brand-secondary">
          <button onClick={handleLogout} className="w-full flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm text-red-300 hover:bg-red-500/20 font-poppins font-semibold">
