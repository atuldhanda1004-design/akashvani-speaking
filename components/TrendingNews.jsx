...
export function SaveButton({ news }) {
  ...
  return (
    <button
      onClick={toggleSave}
      className={`shrink-0 px-2 py-1.5 sm:px-3 sm:py-2 border rounded flex items-center gap-1 text-[10px] sm:text-xs font-poppins transition-colors ${
        isSaved
    ...
    >
      ...
      <span className="hidden sm:inline">{isSaved ? 'सेव्ड' : 'सेव'}</span>
    </button>
  )
}

// Sort helpers: latest first
function sortLiveLatestFirst(updates = []) {
  const t = (s) => new Date(`1970-01-01 ${s || ''}`).getTime() || 0
  return [...updates].sort((a, b) => t(b.time) - t(a.time))
}
...
function LiveUpdateAccordion({ updates }) {
  ...
  const sorted = sortLiveLatestFirst(updates)

  return (
    <div className="relative border border-brand-red rounded-lg p-2 mb-1 mt-2 bg-red-50/30">
      <div className="absolute -top-2 left-2 bg-brand-red text-white text-[9px] font-bold px-1.5 py-0.5 rounded">
        लाइव अपडेट
      </div>

      <div className="flex gap-1.5 items-start mt-1.5">
        <span className="bg-blue-50 text-brand-primary text-[9px] font-bold px-1.5 py-0.5 rounded shrink-0">
          {sorted[0].time}
        </span>
        <p className="text-[10px] sm:text-xs font-yantramanav text-gray-800 leading-tight line-clamp-2">
          {sorted[0].text}
        </p>
      </div>
      ...
      {expanded &&
        sorted.slice(1).map((u, i) => (
          <div
            key={i}
            className="flex gap-1.5 items-start mt-2 pt-2 border-t border-red-100"
          >
            <span className="bg-blue-50 text-brand-primary text-[9px] font-bold px-1.5 py-0.5 rounded shrink-0">
              {u.time}
            </span>
            <p className="text-[10px] sm:text-xs font-yantramanav text-gray-800 leading-tight">
              {u.text}
            </p>
          </div>
        ))}
      ...
      {sorted.length > 1 && (
        <button
          ...
          className="w-full mt-1.5 flex items-center justify-center text-brand-red"
    ...
    <div className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden flex flex-row w-full min-h-[140px] sm:min-h-[180px]">
      ...
      <div className="w-[60%] sm:w-[55%] p-2.5 sm:p-4 flex flex-col justify-between min-w-0">
        ...
        <div className="flex items-center justify-between gap-1.5 mt-2 pt-2 border-t border-gray-100">
          <Link
            href={`/news/${item.slug}`}
            className="flex-1 min-w-0 bg-brand-primary text-white text-[10px] sm:text-xs font-poppins font-semibold py-1.5 sm:py-2 rounded flex items-center justify-center hover:bg-brand-secondary"
          >
            पूरी खबर
          </Link>
          <ShareButtons
...
export default function TrendingNews({
  ...
  const goToPrev = useCallback(
    () => setCurrentSlide((p) => (p === 0 ? news.length - 1 : p - 1)),
    [news.length]
  )
  const goToNext = useCallback(
    () => setCurrentSlide((p) => (p === news.length - 1 ? 0 : p + 1)),
    [news.length]
  )
  ...
  return (
    <section className="mb-6 sm:mb-8">
      <div className="flex items-center gap-2 mb-3">
        <h2 className="text-base sm:text-xl font-bold font-poppins text-brand-primary">
          {title}
        </h2>
      ...
      {layout === 'carousel' ? (
        <div className="relative">
          ...
          {news.length > 1 && (
            <div className="absolute inset-y-0 left-0 right-0 flex items-center justify-between pointer-events-none px-1">
              <button
                onClick={goToPrev}
                className="w-8 h-8 bg-white/90 shadow-lg text-brand-primary rounded-full flex items-center justify-center pointer-events-auto border border-gray-100"
                aria-label="Previous"
              >
        ...
        <div className="space-y-4">
          {news.map((item) => (
            <NewsItemContent key={item.id} item={item} />
          ))}
        </div>
      )}
    </section>
